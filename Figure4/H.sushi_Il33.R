...
plotBedgraph(TCF1_ChIP_DN, chrom = CHR, chromstart = START, chromend = END, addscale = T, range = c(0,1.5), color = "#3A53A4")

par(fig=c(0.4,0.7,0.2,0.3), new=T)
plotBedgraph(CTCF_ChIP_DN, chrom = CHR, chromstart = START, chromend = END, addscale = T, range = c(0,5.5), color = "#ED2224")
...
###### Comparison of contact frequency (in DN3)
setwd('/mnt/data0/sora/TCF1_revision/Reproduce/Figure4/Sushi/selected/')
options(scipen=-100)
...
co1 = "chr7:19350000-19700000"
co2 = "chr7:19700000-19750000"
...
OUT = 'Diff_DN_WT_'
system(paste0("/mnt/data0/apps/anaconda/Anaconda2-5.2/envs/py38/bin/python3 /mnt/data0/sora/bin/py/extract_contact_freq_noSymmetric.py --cool ",DN_WT_cool," --coord1 ",co1," --coord2 ",co2," --norm weight --out ", OUT," --res 5000"))
OUT = 'Diff_DN_KO_'
system(paste0("/mnt/data0/apps/anaconda/Anaconda2-5.2/envs/py38/bin/python3 /mnt/data0/sora/bin/py/extract_contact_freq_noSymmetric.py --cool ",DN_KO_cool," --coord1 ",co1," --coord2 ",co2," --norm weight --out ", OUT," --res 5000"))
...
pdf(file = 'Relb_yellow_box_difference.pdf', width = 3,height = 5)
boxplot(list(DN3_WT = A, DN3_KO = B), las=1, ylab="Normalized contact frequency (balancing)", ylim=c(0,0.01),pch=19,cex=0.5)
